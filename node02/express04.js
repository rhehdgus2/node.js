...
app.use((req, res) => {
    ...
    console.dir(req.header);    // console log과 dir 차이점 : log는 터미널에 출력, dir은 req.header에 전달함
    const userAgent = req.header('User-Agent');
    console.log(userAgent);
    // userAgent 서버요청을 보낼때 디바이스와 브라우저등의 사용자 소프트웨어의
    // 식별정보를 담고 있는 req.header의 종류이고 임의로 수정될 수 없는 값이고
    // 보통은 서버요청시 에러가 발생했을때 요청을 보낸 사용자의 환경을 알아보기 위해 사용

    //http://localhost:3000/?userid=apple
    const paramName = req.query.userid; //GET 방식으로 요청한 변수값을 의미
